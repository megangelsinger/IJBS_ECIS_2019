freqFeat <- function(freq.data){
  ...
  for (w in 1:n_wells){ # For each well,
    ...
    for (f in 1:n_freqs){ # For each frequency,
      res2h[w, f] <- X[6, f] # row/time index 6 corresponds to 2 hour mark, column indicates frequency
      maxres[w, f] <- max(X[, f]) 
...
mainAnalysis <- function(freq.data){
  ...
  return(list(cells = cells, rhos = rhos, singlefeat = singlefeat, pairfeat = pairfeat, triofeat = triofeat,
              pair_rdascan = pair_rdascan, trio_rdascan = trio_rdascan))
